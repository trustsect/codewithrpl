...
class Class extends React.Component {
  handleSubmit = (event) => {
    ...
    const classes = true;
    const open_tag = '<img'
    const close_tag = 'class="codewithrpl"'
    if (code.includes(open_tag)){
      if (code.includes(close_tag)) {
        axios.put(`https://61293f58068adf001789b84b.mockapi.io/api/siswa/${id}`, { name, class: classes })
        .then(res => {
          alert("Sukses")
  ...
  componentDidMount() {
    axios.get(`https://61293f58068adf001789b84b.mockapi.io/api/siswa?class=true`)
      .then(res => {
  ...
  return (
    ...
    <Form onSubmit={this.handleSubmit} className="py-3">
      <h2>Class</h2>
        <p>Buatlah Penulisan Kode Image Dengan Attribut Class Bernama "codewithrpl" HTML Pada Form Di Bawah! <br/>Note: Jangan Menggunakan Attribut Lain.</p>
        <Row>
        <Col sm="auto">
        <Form.Control
          type="text"
