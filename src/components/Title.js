...
class Title extends React.Component {
  handleSubmit = (event) => {
    ...
    const title = true;
    const open_tag = "<title>"
    const close_tag = "</title>"
    if (code.includes(open_tag)){
      if (code.includes(close_tag)) {
        axios.put(`https://61293f58068adf001789b84b.mockapi.io/api/siswa/${id}`, { name, title })
        .then(res => {
          alert("Sukses")
  ...
  componentDidMount() {
    axios.get(`https://61293f58068adf001789b84b.mockapi.io/api/siswa?title=true`)
      .then(res => {
  ...
  return (
    ...
    <Form onSubmit={this.handleSubmit} className="py-3">
      <h2>Title</h2>
        <p>Buatlah Penulisan Kode Title HTML Pada Form Di Bawah!</p>
        <Row>
        <Col sm="auto">
        <Form.Control
          type="text"
