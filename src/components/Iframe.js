...
class Iframe extends React.Component {
  handleSubmit = (event) => {
    ...
    const iframe = true;
    const open_tag = "<iframe src="
    const close_tag = "</iframe>"
    if (code.includes(open_tag)){
      if (code.includes(close_tag)) {
        axios.put(`https://61293f58068adf001789b84b.mockapi.io/api/siswa/${id}`, { name, iframe })
        .then(res => {
          alert("Sukses")
  ...
  componentDidMount() {
    axios.get(`https://61293f58068adf001789b84b.mockapi.io/api/siswa?iframe=true`)
      .then(res => {
  ...
  return (
    ...
    <Form onSubmit={this.handleSubmit} className="py-3">
      <h2>Iframe</h2>
        <p>Buatlah Penulisan Kode Iframe HTML Pada Form Di Bawah! <br/>Note: Jangan Menggunakan Attribut Lain Seperti Id/Class, Kecuali Src!</p>
        <Row>
        <Col sm="auto">
        <Form.Control
          type="text"
