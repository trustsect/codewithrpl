...
class Id extends React.Component {
  handleSubmit = (event) => {
    ...
    const absen = event.target[1].value
    const code = event.target[2].value
    const id = true;
    const open_tag = '<p id="codewithrpl">'
    const close_tag = "</p>"
    if (code.includes(open_tag)){
      if (code.includes(close_tag)) {
        axios.put(`https://61293f58068adf001789b84b.mockapi.io/api/siswa/${absen}`, { name, id })
        .then(res => {
          alert("Sukses")
  ...
  componentDidMount() {
    axios.get(`https://61293f58068adf001789b84b.mockapi.io/api/siswa?id=true`)
      .then(res => {
  ...
  return (
    ...
    <Form onSubmit={this.handleSubmit} className="py-3">
      <h2>Id</h2>
        <p>Buatlah Penulisan Kode Paragraph Dengan Attribut Id Bernama "codewithrpl" HTML Pada Form Di Bawah! <br/>Note: Jangan Menggunakan Attribut Lain.</p>
        <Row>
        <Col sm="auto">
        <Form.Control
          type="text"
