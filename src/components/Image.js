...
class Image extends React.Component {
  handleSubmit = (event) => {
    ...
    const image = true;
    const open_tag = "<img src="
    const close_tag = "width"
    if (code.includes(open_tag)){
      if (code.includes(close_tag)) {
        axios.put(`https://61293f58068adf001789b84b.mockapi.io/api/siswa/${id}`, { name, image })
        .then(res => {
          alert("Sukses")
  ...
  componentDidMount() {
    axios.get(`https://61293f58068adf001789b84b.mockapi.io/api/siswa?image=true`)
      .then(res => {
  ...
  return (
    ...
    <Form onSubmit={this.handleSubmit} className="py-3">
      <h2>Image</h2>
        <p>Buatlah Penulisan Kode Image HTML Pada Form Di Bawah! <br/>Note: Jangan Menggunakan Attribut Lain Seperti Id/Class, Kecuali Width dan Src!</p>
        <Row>
        <Col sm="auto">
        <Form.Control
          type="text"
